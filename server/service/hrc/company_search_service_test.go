...
func TestCompanySearchOnlyReturnsPublicAuditedProfiles(t *testing.T) {
	...
	now := time.Now().Unix()
	visible := &hrcModel.CompanyProfile{UID: 1, CompanyName: companyName("名硕科技"), Audit: 1, UserStatus: 1, Nature: 1, Trade: 2, Scale: 3, District: "北京", DistrictCN: "北京市", NatureCN: "民营企业", TradeCN: "互联网", ScaleCN: "100-499人", ShortDesc: "招聘 Go 工程师", Refreshtime: now}
	...
	require.NoError(t, db.Create(&hrcModel.Jobs{JobsBase: hrcModel.JobsBase{CompanyID: visible.ID, JobsName: "Go 工程师", Audit: 1, Display: 1, Deadline: now + 3600, Refreshtime: now}}).Error)
	require.NoError(t, db.Create(&hrcModel.Jobs{JobsBase: hrcModel.JobsBase{CompanyID: visible.ID, JobsName: "已过期", Audit: 1, Display: 1, Deadline: now - 1}}).Error)
...
func TestCompanySearchDetailAndJobsStayPublic(t *testing.T) {
	...
	require.NoError(t, db.Create(&hrcModel.Jobs{JobsBase: hrcModel.JobsBase{CompanyID: profile.ID, JobsName: "后端工程师", Audit: 1, Display: 1, Deadline: now + 3600, Refreshtime: now}}).Error)
	require.NoError(t, db.Create(&hrcModel.Jobs{JobsBase: hrcModel.JobsBase{CompanyID: profile.ID, JobsName: "暂停职位", Audit: 1, Display: 2, Deadline: now + 3600}}).Error)
