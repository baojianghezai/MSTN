<template>
  <div class="mx-auto max-w-5xl px-4 py-12">
    ...
    <el-dialog v-model="createVisible" title="新增 HR 子账号" width="440px" :close-on-click-modal="false">
      ...
      <p class="text-xs text-slate-400">创建后 HR 可用该手机号 + 密码登录企业中心。</p>
      <template #footer>
...
<script setup lang="ts">
  ...
  const createForm = reactive({ mobile: '', password: '' })
  ...
  const submitCreate = async () => {
    ...
    creating.value = true
    try {
      await createCompanyHR({ mobile: createForm.mobile, password: createForm.password })
  ...
  const handleDelete = async (row: CompanyHRItem) => {
    try {
      await ElMessageBox.confirm(
        `确认移除 HR「${row.mobile}」？移除后该账号将无法登录。`,
        '移除确认',
